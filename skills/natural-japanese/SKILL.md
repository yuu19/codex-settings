...
`/docs`配下の文書、README、運用ガイド、デプロイメモ、プロダクト挙動説明を扱う場合は、本文を書く前に`references/technical-document-policy.md`も読む。この補足は技術文書の章構成やMarkdown形式を定めるものではなく、読者に見える挙動と実装識別子を説明する順序を定める。
...
スクリプトと参照ファイルのパスは、この`SKILL.md`があるディレクトリを基準に解決する。以下のコマンド例は、skillディレクトリをカレントディレクトリとして実行する場合を示す。ユーザーのプロジェクトディレクトリに`./scripts/lint.py`があるとは仮定しない。
