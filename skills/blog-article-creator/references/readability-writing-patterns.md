...
自然な日本語、語順、読点、一文一義、文長、リズム、AI臭さの判断は `natural-japanese` を正本とする。校正の起動条件とsubagent運用は [natural-japanese-proofreading.md](natural-japanese-proofreading.md) に従う。

このreferenceでは、文章を一律に短くする規則を持たない。技術記事として重要な用語を初出で説明し、コード、数式、図表と同じ表記を使うことだけを管理する。
...
- 全体像を先に示し、その後で詳細へ進む。
- 関連する本文、コード、図表は近くに置く。
- 段落が複数の論点を混在させている場合は、意味の単位で分ける。長さだけを理由に分割しない。
