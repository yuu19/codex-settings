...
- 概念、責務、処理、時系列などの関係を説明する図には `$diagram-generator-html` を使う。
- 関数、分布、観測値、シミュレーション結果など数値の正確さが必要なグラフには Matplotlib を使う。
- 実画面が必要な場合は [visual-assets-workflow.md](visual-assets-workflow.md) に従って `browser-agent` で撮影する。
- 専用ツールが利用できない場合は別の方法へ黙って置き換えず、必要な素材を残課題として明示する。
